...
-- A recovery is for a repaired configuration, not an escape hatch from the
-- bounded task-attempt policy.  The first recovery migration intentionally
-- preserves attempt_count; this replacement also refuses exhausted tasks so a
-- later claim can never increment beyond max_attempts.
