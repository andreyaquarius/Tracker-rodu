...
-- The catalogue's public search is deliberately broader than the original
-- record-only text vector, but it must stay a projection of data that is
-- already eligible for public presentation.  In particular, this function
-- never reads Stage 0 ingestion tables or record/source metadata blobs.
--
-- Keep the clearance predicate inside the bounded CTE: evaluating it only
-- while serialising the page could disclose a hidden row through a cursor.
...
-- Reassert the narrow internal ACL after replacing this security-definer
-- implementation.  The public wrappers remain the anonymous API surface.
