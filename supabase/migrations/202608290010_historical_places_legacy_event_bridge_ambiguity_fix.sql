...
-- The apply UPDATE in the original bridge used `candidate` both for the
-- PL/pgSQL record variable and for the temporary-table alias. PostgreSQL then
-- could not decide which `candidate.resolved_place_id` was intended (42702).
-- Keep procedural and relational identifiers distinct throughout the apply
-- path so both existing-place links and newly created-place links can finish.
...
begin
  ...
  update legacy_place_candidates batch_target
  set selected_for_batch = true
  where batch_target.normalized_name in (
    select batch_candidate.normalized_name
    from legacy_place_candidates batch_candidate
    where batch_candidate.match_count <= 1
    order by batch_candidate.first_created_at, batch_candidate.normalized_name
    limit v_limit
  );
  ...
  if coalesce(p_apply, false) then
    for v_candidate in
      ...
      from legacy_place_candidates batch_candidate
      where batch_candidate.selected_for_batch
      order by batch_candidate.first_created_at, batch_candidate.normalized_name
    loop
      v_place_id := v_candidate.matched_place_id;
      if v_place_id is null then
        v_created := security_private.create_project_place_v2(
          p_project_id,
          jsonb_build_object(
            'canonicalName', v_candidate.original_name,
      ...
      update legacy_place_candidates resolution_target
      set resolved_place_id = v_place_id
      where resolution_target.normalized_name = v_candidate.normalized_name;
    end loop;

    update public.person_timeline_events event_row
    set
      place_id = resolved_candidate.resolved_place_id,
    ...
    from legacy_place_candidates resolved_candidate
    where event_row.project_id = p_project_id
      and event_row.place_id is null
      and resolved_candidate.resolved_place_id is not null
      and public.historical_place_search_normalize_v1(event_row.place_name)
        = resolved_candidate.normalized_name;
