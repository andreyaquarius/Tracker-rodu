...
create table if not exists public.app_announcements (
  ...
  category text not null default 'update'
    check (category in ('update', 'feature', 'maintenance', 'tip')),
  media_type text not null default 'none'
    check (media_type in ('none', 'image', 'video', 'link')),
  media_url text,
  cta_label text,
  cta_url text,
  is_published boolean not null default false,
  published_at timestamptz,
  email_status text not null default 'not_planned'
    check (email_status in ('not_planned', 'planned', 'sent')),
  email_requested_at timestamptz,
  email_requested_by uuid references auth.users(id) on delete set null,
...
create index if not exists app_announcements_published_idx
  on public.app_announcements (is_published, published_at desc, created_at desc);
