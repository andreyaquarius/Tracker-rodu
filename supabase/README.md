...
- `GEMINI_API_KEY` — ваш серверний Google Gemini API-ключ для включених
  аналізів гіпотез на тарифах `researcher` і `professional`.
- `GEMINI_MODEL` — необов'язково; модель для серверного ключа. Якщо не
  задано, використовується `gemini-3.5-flash`.
...
На тарифі `free` аналіз гіпотез використовує API-ключ користувача з
налаштувань ШІ-агента. На тарифах `researcher` і `professional` використовується
серверний `GEMINI_API_KEY` з місячними лімітами тарифу.
...
Міграція `202606190015_trial_access.sql` створює тарифні плани, ліміти, підписки,
періодичне використання та аудит. Новий Auth-користувач атомарно отримує 30 днів
можливостей `professional`. Після завершення ефективним стає `free`; дані,
проєкти та членства не видаляються. Старі користувачі автоматично отримують
