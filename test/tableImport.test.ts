...
test("preserves alignment when the first header cell is blank", () => {
  const parsed = parseTableText(
    ",Дата народження,Імя,Батько\n1,08.02.1852,Феодор,Андрій Хмарук",
    "births.csv",
  );
  assert.deepEqual(parsed.headers, ["Колонка 1", "Дата народження", "Імя", "Батько"]);
  assert.equal(parsed.rows[0].values["Колонка 1"], "1");
  assert.equal(parsed.rows[0].values["Дата народження"], "08.02.1852");
