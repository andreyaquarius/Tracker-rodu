...
- `constellationCinema.ts`: immutable theme adaptation, deterministic presentation, smooth camera interpolation and bounded decorative star positions. Family mode starts at the focus person, then visits all loaded unmasked direct ancestors breadth-first: parents, grandparents, then each older generation. Explicit father/mother roles determine the order within each parent's branch; `ancestorOrder` preserves that order even when some parent roles are missing. Shared ancestors appear once, unknown/masked people are skipped, and partners, descendants and collateral relatives are not mixed into the ancestor tour. No sampling is applied to this already bounded (up to 1,000 people) family scene. Time and place tours keep up to 60 highlights sampled across their loaded sequence, including both endpoints; time stops preserve original date qualifiers and place stops honor the current place filter. Stars are decoration, not additional people or actual astronomical coordinates.
